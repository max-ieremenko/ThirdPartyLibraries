...
{
    Task<byte[]?> TryGetPackageFromCacheAsync(string packageName, string version, CancellationToken token);

    Task<byte[]?> TryDownloadPackageAsync(string packageName, string version, CancellationToken token);

    string? ResolvePackageSource(string packageName, string version);
}