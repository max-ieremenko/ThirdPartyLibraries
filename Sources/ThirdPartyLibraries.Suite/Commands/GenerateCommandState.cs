...
internal sealed class GenerateCommandState
{
    ...
    public async Task<ThirdPartyNoticesPackageLicenseContext> GetPackageLicenseAsync(Package package, CancellationToken token)
    {
        ...
        var result = new ThirdPartyNoticesPackageLicenseContext
        {
            ...
            HRef = repositoryLicense.HRefs.FirstOrDefault()
        };

        if (package.TryFindLicense(PackageLicense.SubjectPackage, out var source))
        {
            Name alternativeName = null;
            if (!source.HRef.IsNullOrEmpty())
            {
                result.HRef = source.HRef;
                alternativeName = GetAlternativeNameByHRef(source.HRef);
            }

            if (alternativeName == null
                && package.TryFindLicense(PackageLicense.SubjectRepository, out source)
                && !source.HRef.IsNullOrEmpty())
            {
                alternativeName = GetAlternativeNameByHRef(source.HRef);
            }

            var libraryId = new LibraryId(package.SourceCode, package.Name, package.Version);
            await _licenseFileGroups.AddLicenseAsync(libraryId, package.LicenseCode, alternativeName, token).ConfigureAwait(false);
        }
    ...
    public async Task AlignFileNamesAsync(CancellationToken token)
    {
        ...
        foreach (var (package, license) in _packages)
        {
            ...
            {
                license.FileName = MapToLicensesDirectory(fileName);
            }
            else
            {
                var repositoryLicense = await GetLicensesAsync(package.LicenseCode, token).ConfigureAwait(false);
                if (repositoryLicense.FileNames.Count > 0)
                {
                    license.FileName = repositoryLicense.FileNames[0];
                }
            }
        }
    }
    ...
    private async Task<LicenseIndexJson> LoadLicenseIndexAsync(string code, CancellationToken token)
    {
        ...
        await _licenseFileGroups.AddLicenseAsync(index, token).ConfigureAwait(false);
