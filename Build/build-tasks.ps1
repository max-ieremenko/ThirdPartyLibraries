...
Task Initialize {
    ...
    $script:buildOutApp30Dir = Join-Path $sourceDir "bin\app\netcoreapp3.0"
...
Task Clean {
    ...
    if (Test-Path $buildOutApp30Dir) {
        Remove-Item -Path $buildOutApp30Dir -Recurse -Force
    }
...
Task Build {
    ...
    Exec { & dotnet publish -c Release -f netcoreapp3.0 $solutionFile }
...
Task CreateThirdPartyNotices {
    $app = Join-Path $buildOutApp30Dir ThirdPartyLibraries.exe
...
Task PackGlobalTool {
    ...
    Get-ChildItem -Path $buildOutApp30Dir -File -Recurse | % {$_.LastWriteTime = (Get-Date)}
...
Task PackApp {
    $destination = Join-Path $buildOutDir "ThirdPartyLibraries-net3.0-$packageVersion.zip"
    Compress-Archive -Path "$buildOutApp30Dir\publish\*","$buildOutThirdNoticesDir\*" -DestinationPath $destination
